...
const lz4__namespace = /* @__PURE__ */ _interopNamespaceDefault(lz4);
...
function decompressLz4Block(data) {
  ...
  lz4__namespace.decompressBlock(compressedData, result, 0, compressedData.length, 0);
  return result;
}
...
const _Transport = class _Transport extends websocket.client {
...
_Transport.decodeSync = (buffer) => {
  ...
  if (isLz4Frame) {
    buffer = lz4__namespace.decompress(buffer);
  } else if (buffer[0] === 120 && buffer[1] === 156) {
    buffer = fflate__namespace.unzlibSync(buffer);
  }
